...
import { Donates, itemsResource, priceByCurrencyAction, storeCurrencyAtom, storeItem, Wallets } from "../models/store.model"
...
export const StorePrice = reatomComponent(({ ctx }) => {
  ...
  const getFinishedPrice = (value: number = 0): number => {
    if (!shopItemState || !shopItemState?.type) return 0

    const paymentType = shopItemState.type
  ...
  const finishedPrice = getFinishedPrice(Number(shopItemState?.value ?? 0))
