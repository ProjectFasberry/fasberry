...
import { createPaymentAction, storePrivacyAtom, storeCurrencyAtom, storeItem, storeTargetNickname } from '../models/store.model';
...
const isValidAtom = atom<boolean>((ctx) => {
  ...
  const privacy = ctx.spy(storePrivacyAtom)

  const isValid = shopItemState
    ? (nickname.length >= 1)
    && currency
    && (shopItemState.value !== null)
    && (shopItemState.type !== null)
    && privacy
    : false;
