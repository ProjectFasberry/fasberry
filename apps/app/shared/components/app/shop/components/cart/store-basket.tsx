...
const MenuCartItem = reatomComponent<AtomState<typeof cartDataAtom>[number]>(({ 
  ctx, origin, title, price, img 
}) => {
  return (
    <div className="flex items-center justify-between gap-2 bg-neutral-800 p-2 rounded-lg w-full">
      ...
      <div className="flex items-center gap-1 sm:gap-2">
        ...
        <IconX
          onClick={() => cartDataAtom.removeItem(ctx, origin)}
          size={24}
...
const MenuCartData = reatomComponent(({ ctx }) => {
...
const MenuCartPrice = reatomComponent(({ ctx }) => {
  return (
...
export const MenuCartTrigger = reatomComponent(({ ctx }) => {
  return (
    <HoverCard
