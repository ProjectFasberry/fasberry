import { walletsMap } from "./store-wallets";
import { reatomComponent } from "@reatom/npm-react";
import { Typography } from "@repo/ui/typography";
import { Donates, itemsResource, storeCategoryAtom, storeItem, storeTargetNickname } from "../models/store.model";
...
export const StoreFinishedPreview = reatomComponent(({ctx}) => {
  ...
  if (!shopItemState.type || !walletsMap) return null;

  const paymentType: string = storeCategory === 'donate'
    ? shopItemState.value as "arkhont" | "authentic" | "loyal"
    : shopItemState.type

  const getSelectedDetails = () => {
    switch (shopItemState.type) {
      case "donate":
        const currentDonates = ctx.get(itemsResource.dataAtom) as Donates[]

        const selDonate = currentDonates?.find(cd => cd.origin === shopItemState.value)
