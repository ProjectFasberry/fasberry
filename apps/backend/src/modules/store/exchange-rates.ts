...
import { getExchangeRates } from "./payment/create-crypto-order"
...
export const exchangeRates = new Elysia()
  .get("/exchange-rates", async ({ status }) => {
    ...
    if (!exchangeRatesStr) throw status(HttpStatusEnum.HTTP_500_INTERNAL_SERVER_ERROR)

    const parsed = safeJsonParse<ExchangeRate[]>(exchangeRatesStr)
    if (!parsed.ok) throw status(HttpStatusEnum.HTTP_500_INTERNAL_SERVER_ERROR)
