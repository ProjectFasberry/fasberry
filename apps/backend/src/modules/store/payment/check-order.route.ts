...
export const checkOrderRoute = new Elysia()
  .post('/process-order', async ({ status, ...ctx }) => {
    ...
    if (!signature || !text) {
      return status(HttpStatusEnum.HTTP_400_BAD_REQUEST, throwError('Signature or body is required'))
    }

    const isValid = validateSignatureCryptoPay(text, signature);

    if (!isValid) {
      return status(HttpStatusEnum.HTTP_400_BAD_REQUEST, throwError('Invalid signature'))
    }
    ...
    return status(HttpStatusEnum.HTTP_200_OK, { ok: true })
  }, {
    parse: "text/plain"
  })
